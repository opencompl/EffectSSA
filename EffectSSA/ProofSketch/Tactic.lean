...
**AI DISCLOSURE**: Large parts of this file have been vibe-coded.
-/
...
/-!
## `EqnLemmaUpTo` Reduction

`reduceEqnLemmaUpTo [defs...] at h` unfolds a hypothesis
`h : P.EqnLemmaUpTo hole ρ` — where `P` is a concrete pattern — into a conjunction of concrete equations
between environment lookups and instruction denotations, and then invokes
`grind` to close the current goal.

The list `[defs...]` should contain any user-defined constants needed to expose
the concrete structure of `P` (e.g., `constFoldRw`); they are forwarded to
both the internal `simp` and `grind` invocations.
-/
section EqnLemmaUpToReduction
open Lean Parser.Tactic

syntax (name := reduceEqnLemmaUpTo)
...
macro_rules
  | `(tactic| reduceEqnLemmaUpTo $[[$args,*]]? $[$loc:location]?) => do
    ...
    let grindArgs : TSyntaxArray `Lean.Parser.Tactic.grindParam ← args.mapM fun t =>
      `(Lean.Parser.Tactic.grindParam| $t:term)
    let simpArgs : TSyntaxArray `Lean.Parser.Tactic.simpLemma ← args.mapM fun t =>
      `(Lean.Parser.Tactic.simpLemma| $t:term)
    let grindInstEqn ← `(Lean.Parser.Tactic.grindParam| Inst.EqnLemma)
    let grindAllArgs := #[grindInstEqn] ++ grindArgs
    `(tactic| (
      simp [Pattern.EqnLemmaUpTo, Pattern.EqnLemma, InstSeq.EqnLemma,
            Inst.EqnLemma, $simpArgs,*] $[$loc:location]?
      grind [$grindAllArgs,*]))

end EqnLemmaUpToReduction
