...
-/
public def denote : (is : InstSeq ι) → ITree (InstEff ι) Unit
  | i :: is => Effect.trigger (InstEff ι) i *> denote is
  | [] => .ret ()
...
-/
public def args (is : InstSeq ι) : VarSet :=
  is.argsResults.1

/-- `is.results` is the set of all results of sequence `is`. -/
public def results (is : InstSeq ι) : VarSet :=
  is.argsResults.2
...
-/
public def usesAt (x : VarId) (is : InstSeq ι) : VarSet :=
  { y | is.UsesAt y x }
...
@[grind =, simp] theorem idx_appendLeft :
    (@appendLeft _ is js p).idx = p.idx := by rfl

@[grind =, simp] theorem idx_appendRight :
    (@appendRight _ is js p).idx = p.idx + is.length := by rfl

@[grind =, simp] theorem get_appendLeft {p : PC is} :
    (@appendLeft _ is js p).get = p.get := by grind [get]

@[grind =, simp] theorem get_appendRight {p : PC js} :
    (@appendRight _ is js p).get = p.get := by grind [get]
...
@[simp, grind =] public theorem get_ofAppendLeft :
    (@PC.ofAppendLeft _ is js p).get = p.get := by
  grind [PC.get, PC.ofAppendLeft]

@[simp, grind =] public theorem get_ofAppendRight :
    (@PC.ofAppendRight _ is js p).get = p.get := by
...
-/
@[grind, grind cases]
structure WellFormed (Γ : VarSet) (is : InstSeq ι) where
